...
class Speak(Node):
    def __init__(self):
        ...
        ################################################
        # parameters
        self.tts_engine = self.declare_parameter('tts_engine', xparo_config.get("tts_engine", "pico") )
    ...
    "charger_connected":{
        ...
        "animation": self.animation_unsure #TODO: add new animation
        },
}
    ...
    def speak(self,text):
        ...
        tts_engine = self.get_parameter('tts_engine').get_parameter_value().string_value
        if tts_engine == 'espeak':
            subprocess.run(['espeak', text])
        elif tts_engine == 'festival':
            subprocess.run(['festival', '--tts'], input=text.encode())
        elif tts_engine == 'pico':
            self.stop_existing_playback()
            self.generate_speech(text)
            self.adjust_audio()
            self.play_audio()
        else:
            self.get_logger().error(f'Unsupported TTS engine: {tts_engine}')
        qq = String()
        qq.data = json.dumps({"lazy_text":text})
    ...
    def generate_speech(self,text):
        subprocess.run(['pico2wave', '-w', os.path.join(self.base_path,"tts.wav"), text])

    def adjust_audio(self):
        subprocess.run(['sox',
                         os.path.join(self.base_path,"tts.wav"),
                        os.path.join(self.base_path,"tts_adjusted.wav"),
                        'speed', '0.93'])
    ...
    #     try:
    #         subprocess.run(['aplay','-D','plughw:CARD=sofhdadsp,DEV=0', file_path])
    #     except:
    #         pass
    ...
    #         audio_array = np.frombuffer(audio_data, dtype=np.int16)  # Assuming 16-bit audio
    ...
    def send_command(self, enable, pwm1, pwm2, pwm3, pwm4):
        ...
        #TODO: PIR and light animation are not stable yet
        # if self.arduino.is_open:
